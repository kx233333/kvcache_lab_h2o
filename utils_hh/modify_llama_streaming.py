...
class LlamaAttention_heavy_hitter_streaming(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.initial_budget_ratio = config.initial_ratio
    ...
    def forward(
        ...
        if not self.previous_scores == None:
            ...
            self.initial_budget = int(self.initial_budget_ratio * current_scores_sum.shape[-1])
            self.cache_budget = self.heavy_budget + self.recent_budget + self.initial_budget
        ...
        if attn_tokens_all > self.cache_budget:
            ...
            if not self.recent_budget == 0:
                attn_mask[:, self.initial_budget:-self.recent_budget] = 0
                selected_set = self.previous_scores[:, self.initial_budget:-self.recent_budget]
            else:
                # activate historical best self.cache_budget - self.recent_budget tokens.
                # self.previous_scores # (k-Cache - 1)
                attn_mask[:, initial_budget:] = 0
                selected_set = self.previous_scores[:, self.initial_budget:]
        ...
        score_mask[:, :self.initial_budget] = 1
...
def convert_kvcache_llama_heavy_streaming(model, config):

    for name, module in reversed(model._modules.items()):

        if len(list(module.children())) > 0:
            model._modules[name] = convert_kvcache_llama_heavy_streaming(module, config)

        if isinstance(module, LlamaAttention):
            model._modules[name] = LlamaAttention_heavy_hitter_streaming(config)
